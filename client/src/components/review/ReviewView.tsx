...
export default function ReviewView({ teamId, agentNames, isDemoMode }: ReviewViewProps) {
  ...
  useEffect(() => {
    fetchFeedback();
    fetchPreferences();
  }, [fetchFeedback, fetchPreferences]);
    ...
    <div style={{ maxWidth: '920px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      {showNewEntry && (
        <NewEntryModal
          ...
          onSubmitted={() => { setShowNewEntry(false); fetchFeedback(); }}
        />
      )}
...
function FeedbackRow({ entry, onDelete }: { entry: FeedbackEntry; onDelete?: (id: string) => void }) {
  ...
  return (
    ...
    >
      ...
      {/* Date */}
      <div style={{ flexShrink: 0, fontSize: '9px', color: 'var(--text-muted)', marginTop: '1px', whiteSpace: 'nowrap' }}>
...
function NewEntryModal({ teamId, agentNames, onClose, onSubmitted }: {
  ...
  return (
    <>
      <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(4,6,8,0.7)', zIndex: 200 }} />
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px' }}>
          <span style={{ fontSize: '9px', color: 'var(--text-muted)', letterSpacing: '0.18em', textTransform: 'uppercase' }}>{t('review.modal_title')}</span>

    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          <span style={{ fontSize: 'var(--text-xs)', letterSpacing: '0.15em', color: 'var(--ice)', fontFamily: 'var(--font-mono)', textTransform: 'uppercase' }}>
      ...
      {/* Error */}
      {error && (
        <div style={{ fontSize: 'var(--text-xs)', color: 'var(--crimson)', letterSpacing: '0.06em', padding: '6px 10px', background: 'var(--crimson-bg-subtle)', border: '1px solid var(--crimson-border-subtle)', borderRadius: '3px' }}>
          {error}
        </div>
      )}
      ...
      <div style={{ background: 'var(--surface-1)', border: '1px solid var(--border)', borderRadius: '3px', padding: '12px 14px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
          <span style={{ fontSize: 'var(--text-xs)', color: 'var(--text-muted)', letterSpacing: '0.12em', textTransform: 'uppercase' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          <span style={{ fontSize: 'var(--text-xs)', color: 'var(--amber)', letterSpacing: '0.06em', flexShrink: 0 }}>
          ...
          />
          <span style={{ fontSize: 'var(--text-xs)', color: 'var(--phosphor)', letterSpacing: '0.06em', flexShrink: 0 }}>
      ...
      {showAddInput && (
        <div style={{ display: 'flex', gap: '6px', alignItems: 'flex-start' }}>
          ...
          <button onClick={() => { setShowAddInput(false); setNewRuleText(''); }}
            style={{
              padding: '6px 10px', fontSize: 'var(--text-xs)', letterSpacing: '0.1em',
              fontFamily: 'var(--font-mono)', textTransform: 'uppercase',
      ...
      {suggestions.length > 0 && (
        <div style={{ border: '1px solid var(--ice)33', borderRadius: '3px', padding: '10px', background: 'var(--surface-1)' }}>
          <div style={{ fontSize: 'var(--text-xs)', color: 'var(--ice)', letterSpacing: '0.1em', fontFamily: 'var(--font-mono)', marginBottom: '8px', textTransform: 'uppercase' }}>
          ...
          {suggestions.map((sug, idx) => (
            <div key={idx} style={{ display: 'flex', alignItems: 'flex-start', gap: '8px', padding: '6px 0', borderBottom: idx < suggestions.length - 1 ? '1px solid var(--border)' : 'none' }}>
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ fontSize: 'var(--text-xs)', color: 'var(--text-muted)', marginTop: '2px' }}>{sug.reason} ({sug.supportCount} corrections)</div>
              </div>
              <button onClick={() => handleAcceptSuggestion(idx)}
                style={{
                  padding: '2px 8px', fontSize: 'var(--text-xs)', letterSpacing: '0.08em',
                  fontFamily: 'var(--font-mono)', textTransform: 'uppercase',
                  background: 'var(--phosphor-bg-subtle)', color: 'var(--phosphor)',
      ...
      {rules.length === 0 && !showAddInput ? (
        <div style={{
          ...
          <div style={{ fontSize: 'var(--text-xs)', color: 'var(--text-muted)', opacity: 0.7, lineHeight: 1.5 }}>
      ...
      {/* Saving indicator */}
      {saving && (
        <div style={{ fontSize: 'var(--text-xs)', color: 'var(--text-muted)', letterSpacing: '0.08em', textAlign: 'center' }}>

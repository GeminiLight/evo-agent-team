...
function parseSupervisionSection(guideContent: string): SupervisionConfig {
  ...
  // Find ## Supervision Rules section (until next ## heading, --- divider, or end of string)
  ...
  // Parse threshold from HTML comment
  ...
  // Parse rules: each "- " line is a rule; preceding <!-- sr:{...} --> is metadata
...
router.get('/teams/:id/supervision', async (req, res) => {
  ...
  // Read TEAM_GUIDE.md (teamsDir fallback — tests don't have cwd)
  let guideContent = '';
  try {
    guideContent = await fs.readFile(path.join(getAppConfig().teamsDir, id, 'TEAM_GUIDE.md'), 'utf-8');
  } catch { /* no guide yet */ }

  // Also try cwd-based path (production pattern)
  if (!guideContent) {
    try {
      const configPath = path.join(getAppConfig().teamsDir, id, 'config.json');
      const teamConfig = JSON.parse(await fs.readFile(configPath, 'utf-8'));
...
router.put('/teams/:id/supervision', async (req, res) => {
  ...
  // Read existing TEAM_GUIDE.md — try cwd-based path first, then teamsDir fallback
...
router.post('/teams/:id/supervision/extract', async (req, res) => {
  ...
  // Read correction feedback entries
  const feedbackDir = path.join(getAppConfig().teamsDir, id, 'feedback');
  ...
  // Rule-based extraction: group corrections by common patterns
  ...
  // Pattern 1: Multiple corrections mentioning "delete" or "rm"
  ...
  // Pattern 2: Multiple corrections about tests or testing
  ...
  // Pattern 3: Multiple corrections about config files
  ...
  // Pattern 4: Multiple corrections about git operations
